...
export function DebugPanel() {
  ...
  const runChecks = useCallback(async () => {
    ...
    try {
      ...
      const sysData = sys.ok ? ((await sys.json()) as { mongo?: { ok: boolean; error?: string } }) : null;
      out.push({
  ...
  return (
    <>
      <DashboardListToolbar meta="Run smoke checks against core APIs (read-only).">
            ...
            {results.map((r) => (
              ...
              >
                <div>
                  ...
                  <p className="mt-1 font-mono text-xs text-white/45">{r.detail}</p>
                </div>
                <span
