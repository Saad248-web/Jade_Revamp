...
export default function MenuOverlay() {
  const { isMenuOpen, setMenuOpen, setPartnerOverlayOpen } = useAnimation();

  // Local state for driving secondary menus
    ...
    <AnimatePresence>
      {isMenuOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[50] bg-[#1E2023] flex pt-[60px] md:pt-[80px]" // Static header overlap
        >
                    ...
                    <ul className="flex flex-col space-y-3 lg:space-y-4">
                      ...
                      <li>
                        <button
                          onClick={() => {
                            setMenuOpen(false);
                            setTimeout(() => setPartnerOverlayOpen(true), 300);
                          }}
                          className="text-left"
                        >
                          <span className="block text-3xl lg:text-4xl font-philosopher text-transparent bg-clip-text bg-gradient-to-r from-[#EFCD62] from-50% to-white to-50% bg-[length:200%_100%] bg-right hover:bg-left transition-all duration-500 ease-out">
          ...
          <div className="absolute lg:hidden bottom-0 left-0 right-0 border-t border-white/10 bg-[#2C2E31] z-10 flex">
            <div
              className="flex-1 py-4 flex flex-col items-center gap-1.5 opacity-50 cursor-pointer"
              onClick={() => {
                setMenuOpen(false);
