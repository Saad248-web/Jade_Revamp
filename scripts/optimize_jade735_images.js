...
const LOG_FILE =
  "C:\\Users\\Admin\\Desktop\\Jade_ReVamp\\jade735_optimization.log";
...
async function main() {
  log("=== Jade 735 Villa — Image Optimization ===");
  ...
  const ROOT =
    "C:\\Users\\Admin\\Desktop\\Jade_ReVamp\\public\\Villa_Retreats\\Jade 735";

  const FOLDERS = ["3-Experience", "4-Perfect for"];
  ...
  for (const folder of FOLDERS) {
    ...
    for (let filename of entries) {
      let srcPath = path.join(dir, filename);
      ...
      // If there's an awkward double extension like .jpg.webp, .webp check caught it anyway,
      // but let's handle .HEIC specially if it's misnamed JPEG by simply reading it as a buffer
      if (ext === ".heic") {
        const newFilename = filename.replace(/\.heic$/i, ".jpg");
        const newSrcPath = path.join(dir, newFilename);
        fs.renameSync(srcPath, newSrcPath);
        filename = newFilename;
        srcPath = newSrcPath;
        ext = ".jpg";
        log(`Renamed .HEIC to .jpg -> ${filename}`);
      }
